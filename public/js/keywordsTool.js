// File: public/js/keywordsTool.js (Đã sửa lỗi sử dụng hàm)
...
            setLoading(searchButton, true, "Đang tìm kiếm..."); // SỬ DỤNG IMPORT
            const inputs = document.querySelectorAll('#page-keywords .video-url-input'); 
...
                const videoId = getYoutubeId(url); // SỬ DỤNG IMPORT
...
            setLoading(searchButton, false, "Tìm kiếm Thông Tin Video"); // SỬ DỤNG IMPORT
        });
    }

    if (generateKeywordsButton) { 
        generateKeywordsButton.addEventListener('click', async () => {
            setLoading(generateKeywordsButton, true, "Đang tạo..."); // SỬ DỤNG IMPORT
...
                setLoading(generateKeywordsButton, false, "Tạo Bộ Từ Khoá Của Chủ Đề"); // SỬ DỤNG IMPORT
                return;
            }
...
                btn.onclick = () => copyToClipboard(btn.dataset.copyText, btn, true); // SỬ DỤNG IMPORT
            });
...
            copyButton.addEventListener('click', () => copyToClipboard(copyButton.dataset.copyText, copyButton)); // SỬ DỤNG IMPORT
