...
<head>
    ...
    <title>Bộ Công Cụ YouTube (Từ Khoá & Comment)</title>
    <!-- Tải Tailwind CSS -->
    ...
    <style>
        ...
        #page-hub { display: block; } /* Trang chủ luôn hiển thị ban đầu */
        ...
        /* --- CSS CHO CÔNG CỤ TỪ KHÓA (Tái sử dụng) --- */
        .popover {
        ...
        .lang-dropdown {
            display: none; position: absolute; bottom: 100%; /* Hiển thị bên trên nút */
...
<body class="bg-gray-100 text-gray-800">
    ...
    <div id="page-hub" class="page active container mx-auto p-4 max-w-4xl">
        <h1 class="text-4xl font-bold text-center text-gray-800 mb-8">Bộ Công Cụ YouTube</h1>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <!-- Card 1: Công cụ Từ khoá -->
            <div id="hub-card-keywords" class="hub-card" data-page="page-keywords">
                <h2>🚀 Công Cụ Từ Khoá SEO</h2>
                <p>Phân tích video và tạo bộ từ khoá, tiêu đề, mô tả chuẩn SEO cho chủ đề của bạn.</p>
            </div>
            <!-- Card 2: Công cụ Comment -->
            <div id="hub-card-comments" class="hub-card" data-page="page-comments">
                <h2>📝 Trình Lấy Comment</h2>
                <p>Lấy toàn bộ bình luận từ video, tự động dịch và sắp xếp theo lượt thích.</p>
            </div>
        </div>
    </div>

    <!-- Trang 1: Công Cụ Phân Tích YouTube & Tạo Từ Khoá (Ẩn) -->
    ...
    <!-- Trang 2: Trình Lấy Comment YouTube (Ẩn) -->
    <div id="page-comments" class="page container mx-auto p-4">
        ...
        <div class="comment-tool-container">
            <h1>📝 Trình Lấy Comment YouTube</h1>
            <!-- Hàng nhập URL video -->
            <div class="comment-row">
                <div class="field"><input id="comment-video-url" class="comment-input" type="text" placeholder="Dán link video YouTube vào đây..."></div>
                
                <!-- Nút chọn ngôn ngữ MỚI -->
                <div class="relative">
                    <button id="lang-select-btn" class="lang-select-btn" type="button">
                        Dịch sang: <span>Tiếng Việt</span>
                    ...
                    <div id="lang-dropdown" class="lang-dropdown">
                        <!-- Các ngôn ngữ sẽ được thêm bằng JS -->
                    </div>
                </div>

                <button id="comment-search-button" class="comment-btn">Tìm kiếm</button>
            </div>
            <!-- Thanh sắp xếp -->
            <div class="sort-bar" id="comment-sort-bar">
                <span class="sort-title">Sắp xếp theo:</span>
                <button class="sort-btn" id="sort-like">Like ↓</button>
                <button class="sort-btn" id="sort-replies">Cmt cấp 2 ↓</button>
                <button class="sort-btn" id="sort-date">Ngày ↓</button>
                <span class="sort-hint">Bấm lại nút để đảo chiều ↑/↓</span>
            </div>
    ...
    <script>
        ...
        function initKeywordsTool() {
            ...
            function createInputRows() {
                if (!videoInputsContainer) return; // Kiểm tra nếu không ở đúng trang
            ...
            function updatePopover(popoverElement, data) {
                ...
                if (data && !data.error) {
                    ...
                    const hashtagRegex = /#[\w\d_]+/g; // Sửa lại regex cho hashtag
        ...
        function initCommentsTool() {
            ...
            let allCommentTexts = []; // Mảng chứa toàn bộ văn bản gốc
            let allTranslations = []; // Mảng chứa toàn bộ văn bản dịch
            let currentTranslationHeader = 'Bản dịch (vi)'; // Biến lưu tiêu đề cột dịch

            // ===== Translation (Backend) =====
            async function translateBatchToVi(texts, langCode) {
                if (!texts || texts.length === 0) return [];
                
                // Cập nhật tiêu đề cột dịch
                // Lấy tên ngôn ngữ từ mã code
                const langName = languages.find(l => l.code === langCode)?.name || langCode;
                currentTranslationHeader = `Bản dịch (${langName})`;
                statusDiv.textContent = `Đang dịch ${texts.length} bình luận sang ${langName}... (chỉ 1 lần)`;
                
                try {
                    const response = await fetch('/api/translate', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ texts: texts, targetLang: langCode }) // Gửi ngôn ngữ đã chọn
                    });
                    
                    if (!response.ok) {
                        const err = await response.json();
                        throw new Error(err.message || 'Lỗi dịch');
                    }
                    
                    const data = await response.json();
                    return data.translations || texts.map(() => '(Chưa dịch)');

                } catch (e) {
                    console.error('Lỗi gọi backend dịch:', e);
                    statusDiv.textContent = `Lỗi dịch: ${e.message}`;
                    return texts.map(() => '(Lỗi dịch)');
                }
            }
            ...
            if(searchButton) { // Chỉ gán sự kiện nếu nút tồn tại
                searchButton.addEventListener('click', async () => {
                    ...
                    searchButton.innerHTML = '<div class="loader" style="width: 20px; height: 20px; border-width: 3px;"></div>'; // Loader nhỏ hơn
                    ...
                    try {
                        ...
                        if (allThreads.length === 0) {
                            ...
                            searchButton.disabled = false; searchButton.textContent = 'Tìm kiếm'; // Phục hồi nút
                            return;
                        }
                        
                        // 2. TỐI ƯU: Dịch tất cả 1 lần (sử dụng targetLang)
                        allTranslations = await translateBatchToVi(allCommentTexts, targetLang);

                        // 3. Hiển thị
                        statusDiv.textContent = `Đã tải và dịch xong ${allThreads.length} luồng bình luận!`;
                        attachSortHandlers();
                        sortBar.style.display = 'flex';
                        renderThreads(); // Render lần đầu (đã dịch)
                        
                    } catch (e) {
                        console.error('Lỗi:', e);
                        statusDiv.textContent = `Đã xảy ra lỗi: ${e.message}`;
                    } finally {
                        searchButton.disabled = false; searchButton.textContent = 'Tìm kiếm';
                    }
                });
            }
            ...
            async function fetchAllComments(videoId, pageToken = '') {
                ...
                if (!response.ok) {
                    const errText = await response.text(); // Lấy lỗi text
                    console.error("Lỗi fetchAllComments:", errText);
                    let errMessage = 'Lỗi không xác định';
                    try {
                        const errJson = JSON.parse(errText); // Thử parse JSON
                        errMessage = errJson.message || errText;
                    } catch(e) {
                        errMessage = errText; // Nếu không phải JSON, dùng text
                    }
            ...
            // TỐI ƯU: Tách biệt việc Thu thập (Collect) và Dịch (Translate)
            function collectThreads(items) {
                items.forEach(item => {
                    ...
                    const threadObj = {
                        ...
                        translationIndexStart: allCommentTexts.length // Vị trí bắt đầu của bản dịch
                    };

                    // Thu thập văn bản gốc (top level)
                    allCommentTexts.push(top.textOriginal || '');
                    
                    // Thu thập văn bản gốc (replies)
            ...
            function renderThreads() {
                ...
                for (const obj of arr) {
                    // Bây giờ chỉ cần tạo DOM, không cần chờ dịch
            ...
            function createThreadDOM(threadObj) {
                ...
                const dateText = publishedDate ? publishedDate.toLocaleString('vi-VN', { hour12: false }) : '—';
            ...
            function createCommentElement(snippet, isReply = false, overrideText = null) {
                ...
                if (overrideText !== null) { 
                    text.textContent = overrideText; // Hiển thị text thuần (đã dịch)
                } else { 
                    text.innerHTML = snippet.textDisplay || snippet.textOriginal; // Hiển thị HTML gốc
                }
